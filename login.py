...
def pdf_maker(month):
    ...
    if cur.rowcount > 0:
        ...
        # Continue processing the info list
    else:
        print("No results found.")
    html = f"""
    <h1><center>Pay Slip - {company_name}</center></h1>
    <h3><center>{month} - 2023</center></h3>
    ...
    <p>
        <b>Address:</b> 121 Brunel Rd, Mississauga, ON L4Z 3E9 <br>
    </p>
    <br>
    <p><i> Employee Signature __________________________ {info[1]}(Date of print: {str(datetime.datetime.now())}) </i></p>
    """
...
def connect():
    global myconn
    global cur
    myconn = mysql.connector.connect(host=HOST, port=3306, user=USER, passwd=PASSWD, database="company")
    cur = myconn.cursor(buffered=True)

def new_connect():
    tempMyconn = mysql.connector.connect(host=HOST, port=3306, user=USER, passwd=PASSWD,database="company")
